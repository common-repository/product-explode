=== Plugin Name ===
Contributors: (this should be a list of wordpress.org userid's)
Tags: Product Explode Plugin,prdct_expld,Product Plugin
Requires at least: 5.0
Tested up to: 6.4.2
Stable tag: 2.0
License: GPLv2 or later
License URI: https://www.gnu.org/licenses/gpl-2.0.html

-- short description here --

Product Explode plugin will give you the extension or extra panel on product menu on WooCommerce admin side.

== Description ==

Product Explode plugin will give you the extension or extra panel on product menu on WordPress admin side. In that panel, you will be able to see the list of single products under Grouped product Image with labels on wordpress admin side. You can give the size and name of those labels. You can drag those labels from table to product image to link or highlight the area of the specific products in the grouped product. It will override the layout of single-product.php page of woocommerce plugin to show the extra product table with labels. It also shows the quantity and add to cart option from where you can directly add the product to your cart.

**Features**

* Store administrator can handle the singles products with grouped product.
* The plugin will displays the number of labels according to those single products in table on WordPress side.
* Customer can highlight applicable products by clicking on special areas or links on the product image. 

= Product Explode Shortcodes =


== Installation ==

**Using FTP**

1. Download the woocommerce plugin.
2. Upload the entire **Woocommerce** folder to the wp-content/plugins/** directory.
3. Activate the plugin through the **Plugins** menu in WordPress admin.
4. Upload the entire **Product Explode** folder to the **/wp-content/plugins/** directory.
5. Activate the plugin through the **Plugins** menu in WordPress admin.
6. Go to **Products** tab from side menu in Wordpress admin side.
7. Go to **Product Explode** panel

**Uploading in WordPress Dashboard**

1. Download `woocommerce.zip`.
2. Download `product_explode.zip`.
3. Navigate to the 'Add New' in the plugins dashboard.
4. Navigate to the 'Upload' area.
5. Select `woocommerce.zip` and `product_explode.zip`from your computer.
6. Click 'Install Now'.
7. Activate both plugins in the Plugin dashboard.

== Screenshots ==


== Upgrade Notice ==

= 0.1 =

Starter version no upgrade is required.

== Frequently Asked Questions ==

Que. How to use this plugin?
Ans. * Activate this plugin by following the installation steps
     * Go to Product menu on WP side.
     * Create new or select an existing Grouped Product.
     * Select Product Explode panel.
     * It will show you the Product image with table and also the lables with number.
     * You can arrange the labels according to single product.
     * You can manage the size of labels and give the text on those labels.

== Changelog ==

= Version 2.0 -Jan-2024 =
* add plugin short description in readme.txt
* On activation we need to check if WooCommerce plugin is activated or not
* On activation we need to check if group product plugin is activated or not
* Error : It is working only when minimum Group Product is added other wise giving error
* Error : Even after adding a group product it required to added featured image other wise giving error
* Front : SKU and Prod Name Not showing on front side. Columns are not proper
* Front : "Add to cart" button is working commonly for all so it should to display once only
* Front : For Dark Background image, Text is not Visible. We should give option to change text color etc.
* Front : Hover effect on exploded View
* Front : Add no image
* Front design issue qty field
* CSS change on Explodede Box
* Notice to add Featured Image
* Restricted Explode elements if Featured Image not uploaded

= Version 0.1 -july-2018 =
* New: Store administrator can Specify the single products of a grouped product by using labels
* New: Store administrator can change the size of labels.
* New: Store administrator can add small text on labels.
* New: User can see the same product image on front end with the labels and texts.



